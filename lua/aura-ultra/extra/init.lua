...
function M.setup()
  local aura_ultra = require("aura-ultra")
  ...
  -- aura-ultra.setup({ plugins = { all = true } })
  for _, extra in ipairs(names) do
    local info = M.extras[extra]
    local plugin = require("aura-ultra.extra." .. extra)
    for _, style in ipairs(style_keys) do
      local style_name = styles[style]
      local colors, groups, opts = aura_ultra.load({ style = style, plugins = { all = true } })
      local fname = extra
      ...
      colors["_upstream_url"] = "https://github.com/minhtri2710/aura-ultra.nvim/raw/main/extras/" .. fname
      colors["_style_name"] = "Aura Ultra" .. style_name
      colors["_name"] = "aura-ultra_" .. style
      colors["_style"] = style
      print("[write] " .. fname)
