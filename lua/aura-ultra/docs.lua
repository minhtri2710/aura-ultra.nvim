...
local Groups = require("aura-ultra.groups")
...
function M.extras()
  local Extra = require("aura-ultra.extra")
...
function M.plugins()
  ...
  for _, name in ipairs(names) do
    ...
    t[#t + 1] = {
      link(repo, group.url),
      link(("`%s`"):format(name), "lua/aura-ultra/groups/" .. name .. ".lua"),
    }
  end
  return M.table(t)
end
...
function M.update()
  local config = Docs.extract("lua/aura-ultra/config.lua", "\n(--@class aura-ultra%.Config.-\n})")
