...
function M.brighten(color, lightness_amount, saturation_amount)
  ...
  local hsluv = require("aura-ultra.hsluv")
...
---@param groups aura-ultra.Highlights
...
function M.cache.file(key)
  return vim.fn.stdpath("cache") .. "/aura-ultra-" .. key .. ".json"
end

---@param key string
function M.cache.read(key)
  ---@type boolean, aura-ultra.Cache
...
function M.cache.clear()
  ...
  while true do
    ...
    if ftype == "file" and name:match("^aura%-ultra%-.+%.json$") then
      local ok, err = uv.fs_unlink(cache_dir .. "/" .. name)
      if not ok then
        local notify = vim.notify
        if notify then
          vim.schedule(function()
            notify(string.format("aura-ultra: failed to remove cache '%s': %s", name, err), vim.log.levels.WARN)
          end)
        end
      end
    end
